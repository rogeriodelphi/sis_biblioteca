...
from biblioteca.models import Emprestimo, Biblioteca
...
#    if request.user.is_authenticated() and not request.user.groups.filter(name='nome'):
#        return HttpResponseForbidden('Acesso Negado')

    title = 'Minhas Bibliotecas'
    bibliotecas = Biblioteca.objects.filter().order_by('nome')

    return render(request, 'bibliotecas.html', locals())
